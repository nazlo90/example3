...
class BE_Gallery_Metabox
{
	...
	public function admin_scripts() {
		...
		wp_register_script( 'gallery-metabox-ajax',get_template_directory_uri().'/js/gallery-metabox-ajax.js' , array( 'jquery' ), null, true );
		wp_register_style( 'gallery-metabox-style', get_template_directory_uri().'/css/gallery-metabox-style.css', array(), null, 'all' );
	...
	public function metabox_add() {
		...
		$post_types		= apply_filters( 'be_gallery_metabox_post_types',$this->post_type);
	...
	public function gallery_display( $loop ) {
		...
		foreach( $loop as $image ):
			...
			// removal button
			$gallery .= apply_filters( 'be_gallery_metabox_remove', '<span class="be-image-remove" rel="' . $image->ID .'"><img src="' . get_template_directory_uri().'/img/cross-circle.png' . '" alt="Remove Image" title="Remove Image"></span>' );
